...
class Column[Name, T](using val name: DbIdentifier[Name]) extends Field[T]:
...
object ColumnsBuilder:
  ...
  given head[Name, T, Tail <: Tuple](using dbi: DbIdentifier[Name], builder: ColumnsBuilder[Tail]): ColumnsBuilder[Column[Name, T] *: Tail] with

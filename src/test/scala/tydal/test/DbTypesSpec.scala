...
object all_supported_types extends TableSchema[
  "all_supported_types",
  (
    "varchar" :=: varchar,
...
class DbTypesSpec extends AnyFreeSpec with should.Matchers with IntegrationTesting:
  ...
  val insertAllSupportedTypes =
    session.flatMap(_.prepare(Insert.into(all_supported_types).compile))

  val selectAllSupportedTypes =
    session.flatMap(_.prepare(Select.from(all_supported_types as "a").take(_("a").*).where(_("a", "uuid") === "uuid?").compile))
  ...
  "Arrays" - {
    ...
    "array[numeric(precision, scale)]" ignore List(BigDecimal(2.2), BigDecimal(3.3)).testArr[numericOf[16, 2]]
  }
  ...
  "All supported types" in {
    ...
    val actual = (for {
      _ <- insertAllSupportedTypes.use(_.execute(
        "varchar" ~~> "hello",
        "varcharOf[16]" ~~> "hello",
        "int2" ~~> 2.toShort,
        "int4" ~~> 2,

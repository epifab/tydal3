...
object NullableAlgebraSpec:
  ...
  class $[A, B]:
    def everyNullableAlgebra[X](using EveryNullableAlgebra[A, B, X]): test[X] = new test[X]
    def anyNullableAlgebra[X](using AnyNullableAlgebra[A, B, X]): test[X] = new test[X]
